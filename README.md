...
打分器有一份离线可复现的成绩单，跑一下就能看到（不需要 API Key）：

```bash
python -m vidrecap eval
```
...
考卷在 `vidrecap/monitor/evals/cases/scorer_v1.jsonl`：49 条手写中文样例，
三类病句（清晰度/通顺度/完整度）各 8 条坏例 + 4 条好句对照，另加 5 条多重问题、
8 条好句（含"很短但完整""较长但清晰"这类对抗性样例）。
低于基线时命令会以非零码退出，所以它可以直接当门禁用。

> **成绩要打个折扣看**：这套考卷和打分规则出自同一作者，满分只说明"规则与考卷自洽"，
> 不代表真实场景的准确率。它的真正价值有两点：一是任何改动让质量退步，CI 立刻拦下；
> 二是将来接真实大模型打分器时，可以在同一批题上横向比分数。
