# 开发计划（路线图）

> 本文件是**规则层**资产，描述接下来几次提交的目标、产出、验收标准与"不做什么"。
> 分层规矩见 [AGENTS.md](../AGENTS.md)，架构讲解见 [ARCHITECTURE.md](ARCHITECTURE.md)。

## 总原则

1. **考卷先行**：先定义"什么算好句子"的评测集，再写实现——标准先立，实现围绕标准写。
2. **每次提交自带验收**：不写"以后再补测试"，测试与实现同一次提交落地。
3. **行为不变的重构要有证据**：不改变行为的提交，必须证明 `vidrecap demo` 输出与改前逐字节一致。
4. **骨架已就位**：函数签名与契约已写好，后续提交只填实现、补测试，不再改结构。

## 依赖关系

```
提交1（打分器）──→ 提交2（考卷+跑分）──┐
        │                              ├──→ 提交4（注毒+端到端）
        └────────→ 提交3（语义修正）────┘
```

提交 2 与 3 都依赖提交 1，彼此独立，可以调换顺序；提交 4 依赖 3。

---

## 提交 1：规则层三指标打分器 ✅ 已完成

**目标**：能对一句话给出可解释的质量分——这是后续"低于阈值就修"的判据。

**产出文件**

| 文件 | 动作 |
|---|---|
| `vidrecap/rules/quality.py` | 已实现：三指标规则 + 加权总分 + 扣分原因 + 达标判定 |
| `vidrecap/rules/api/__init__.py` | 已挂出 `HeuristicScorer` / `is_acceptable` / `unacceptable_reason` / `SEVERE_METRIC_FLOOR` |
| `tests/test_quality.py` | 新建 34 个测试：三指标正反例、加权公式、确定性、闸门、插座形状 |

**三指标规则**（每条扣分都写进 `reasons`，可以直接讲给业务方听）

| 指标 | 规则 | 扣分 |
|---|---|---|
| 清晰度 | 句首直接是谓语（"介绍了…"）→ 缺主语 | −0.60 |
| | 指代词过密（他/这个/那个；"其他"这类词不算） | 按比例，最多 −0.55 |
| 通顺度 | 相邻两字词重复（"介绍介绍了"） | −0.60 |
| | 虚词重复（了了、的的） | −0.40 |
| | 标点连用 | −0.30 |
| | 长句中间没有停顿标点（连读） | −0.25 |
| | 句子过短（< 6 字）/ 过长（> 80 字） | −0.25 / −0.20 |
| 完整度 | 结尾没有终止标点（被截断） | −0.55 |
| | 以停顿标点结尾（话说一半） | −0.55 |
| | 看不出谓语（结构不成型） | −0.30 |

**闸门：实现时发现的关键设计。** 只用"加权总分 ≥ 阈值"是不够的——实测发现
缺主语（总分 0.70）、指代模糊（0.72）、词语重复（0.82）、截断（0.89）**光看总分全部过线**，
因为通顺度只占 0.3、完整度只占 0.2，单项全崩也拉不垮加权平均。
因此达标判定加了一条**一票否决**：任一指标低于 0.5（`SEVERE_METRIC_FLOOR`），整句判为需修正。
标准只写在规则层一处，规划层挑句子、评测算准确率都调它——线上判的与考卷量的是同一个标准。

**刻意不做**：完整度不查"是否覆盖原文全部信息"（概括本来就该丢细节，用覆盖率会误伤好句），
所以 `context` 参数在当前启发式实现里不参与打分；模型档实现会用它。
另有一个专门的测试锁住这条决定：换一个长短不同的原文，同一句话的分数必须完全相同。

**验收**：已完成。打分测试 34 个全绿；全库 147 个测试全绿；demo 输出与最初基准逐字节一致。
**不做什么**：不碰服务层、不改调度器、不做修正。

---

## 提交 2：A 层考卷 + 跑分器 + `vidrecap eval` ✅ 已完成

**目标**：给打分器一张考卷和一份成绩单，并让成绩单进 CI 当门槛。

**产出文件**

| 文件 | 动作 |
|---|---|
| `vidrecap/monitor/evals/cases/scorer_v1.jsonl` | 新建：49 条手写考卷 |
| `vidrecap/monitor/evals/loader.py` | 已实现：JSONL 加载（容忍 CRLF、注释行、空行，报错带行号，查重复 id） |
| `vidrecap/monitor/evals/metrics.py` | 已实现：阈值准确率、排序正确率、分类别准确率、错题清单 |
| `vidrecap/monitor/evals/runner.py` | 已实现：`run_scorer_eval` / `run_eval` / `baseline_checks` |
| `vidrecap/user/cli.py` | 新增 `vidrecap eval` 子命令，低于基线时退出码非零 |
| `vidrecap/rules/baselines.py` | 按首次实测校准（0.75/0.90 → 0.95/0.95） |
| `tests/test_evals.py` | 新建 23 个测试：加载器、指标算法、成绩单、基线门槛、命令行 |
| `README.md` | 发榜：公布实测成绩与"考卷要打折扣看"的说明 |

**考卷配比**：清晰度 8 坏 + 4 好、通顺度 8 坏 + 4 好、完整度 8 坏 + 4 好、
混合 5 坏、好句对照 8 好，共 49 条。

**指标定义**

- **阈值准确率** = 判"过/不过"与标准答案一致条数 ÷ 总条数。
  判的依据是**规则层的达标判定**（阈值 + 单项一票否决），不是裸的总分过线——
  线上闸门用的就是这套判定，考卷必须量同一个东西。
- **排序正确率** = 所有（好句, 坏句）配对中"好句分更高"的比例，同分算半分；
- **分类别准确率** = 按 `category` 拆开的阈值准确率，看在哪类病句上翻车。

**首次实测（提交 2 完成时）**：49/49 全对，阈值准确率 100%、排序正确率 100%。
基线随之上调为 **0.95 / 0.95**——不设成满分，是因为门槛一旦等于满分，
规则的小改动一动就红，反而会诱导别人去改考卷而不是改实现。

**实现时的两点调整**

1. **规则分了两档**：硬缺陷（缺主语、指代过密、词重复、虚词重复、标点连用、
   被截断、话说一半、碎片、严重连读）各自扣分都超过 0.5，单独命中一条就足以
   判不合格；轻扣（稍长、略短、看不出谓语、轻度连读）只做辅助信号。
   依据是：三个指标加权平均、权重都小于 1，单条扣分不到 0.5 就等于没有闸门作用。
2. **基线对照挪进监控层**（`baseline_checks`）：命令行原先要直接引用规则层拿基线数值，
   这违反依赖方向；现在由监控层给出"哪条不达标"，用户层只负责打印。

**刻意不做**：不写修正器；不接真实模型（留给路线图）。

---

## 提交 3：语义修正（规划 / 服务 / 规则 / 数据 联动）✅ 已完成

**目标**：低于阈值的句子被修正，且**绝不引入原文没有的事实**。

**产出文件**

| 文件 | 动作 |
|---|---|
| `vidrecap/planning/sentences.py` | 已实现：共享断句纯函数（外部层有一份等价副本，对照测试守住一致性） |
| `vidrecap/planning/corrections.py` | 已实现：按规则层达标判定挑出待修句，原因写进计划 |
| `vidrecap/rules/guardrail.py` | 已实现：实体抽取 + 忠实性校验（原文片段拼装核对，容忍连接字与一字之差） |
| `vidrecap/service/corrector.py` | 已实现：拆句 → 打分 → 按计划修正 → 重打分 → 护栏 → 回退 |
| `vidrecap/service/orchestrator.py` | 已接入：`scorer` / `corrector` / `quality_config` 可选参数（默认 None，行为不变） |
| `vidrecap/data/models.py` | 已补 `PartialSummary.avg_quality` / `corrected_count`、`RecapStats` 汇总字段 |
| `vidrecap/monitor/evals/cases/corrector_v1.jsonl` | 新建：15 条修正考卷（12 该修 + 3 不该动的对照） |
| `vidrecap/monitor/evals/*` | 已实现：修正器指标与跑分器（**直接跑线上编排**，不是另写评测版） |
| `vidrecap/external/adapters/demo/corrector.py` | 已实现：抽取式确定性修正器（**从提交 4 提前**，没有它修正链路无法演示） |
| `tests/test_corrector.py` | 新建 20 个测试：断句一致性、计划、护栏、回退、降级、流水线接入 |

**回退优先原则**（已实现并测住）：护栏不过、修正器抛异常、或分数没变好——
一律保留原句。无幻觉率、不倒退率、好句不动率三项基线因此都是满分，靠机制保证。

**首次实测**：修正成功率 12/12、无幻觉率 100%、不倒退率 100%、好句不动率 100%。

**实现时被考卷抓出的真问题**（评测第一次发挥价值）：corr-011 的连读句修正后
被护栏误判越界——根因是"整句 34 字被当成一个实体"，拼装核对只差末尾一个字
就整体判负。修法是**拼接核对容忍至多一个对不上的字**（那是分词边界，不是捏造），
两个以上对不上才算越界。顺手把该题加长到 50 字以上，让它真正走到修正路径。

**已知局限**（文档写明，测试锁住）：同义改写（原文"缺席"、修正写"没到场"）
仍会被护栏误判回退——宁严勿松是有意的；真实场景换模型档护栏，见 docs/REUSE.md。

**验收**：全部达成；`vidrecap demo` 输出与提交前逐字节一致。
**不做什么**：不改提示词、不接真实模型、不动压缩与分片逻辑。

---

## 提交 4：demo 注毒 + 端到端评测 + 命令行开关 ✅ 已完成

**目标**：让整条链路在"有烂句"的情况下也能被验证，且保持确定性。

**产出文件**

| 文件 | 动作 |
|---|---|
| `vidrecap/external/adapters/demo/source.py` | 已实现 `poison_rate`：三种手法按句子序号确定性轮换（丢主语 / 动词重复 / 截断） |
| `vidrecap/user/cli.py` | 已实现 `--poison [比例]` / `--no-correct` / `--threshold` / `--weights` |
| `tests/test_end_to_end.py` | 新建 4 条端到端断言 |

**与规划的一处偏离**：`monitor/evals/scenarios.py` 不再单设——端到端断言没有
生产调用方，纯属测试，按 YAGNI 直接放在 `tests/test_end_to_end.py`。

**行为变化（有意的）**：demo 默认开启质检修正；原行为用 `--no-correct` 复现，
已验证与最初基准**逐字节一致**。统计行追加"修正 N 句 | 平均质量 X"。

**端到端断言**（全部达成）：注毒时修正真的发生；最终概括无原文外实体
（合并用的"[片段N]"序号标签剔除后核对）；修正让平均分上升；同一输入两次
运行输出一致（`elapsed_sec` 是真实计时，不参与比较）。

**验收**：196 个测试全绿。

---

## 阶段二（提交 5–8）：从"能跑"到"能用"

引擎侧（分片、压缩、质检、修正、评测）已经闭环，但吃的还是假数据。阶段二让它第一次
接触真实输入与真实模型，同时守住两条底线：**零 Key 仍然可跑**、**默认 demo 行为不变**
（每次提交都要证明 `vidrecap demo` 默认输出与改前逐字节一致）。

```
提交5（SRT 字幕源）──┐
                      ├──→ 提交7（失败重试与降级）──→ 提交8（断点续跑）
提交6（真实模型）────┘
```

提交 5 与 6 相互独立可换序；提交 7 与 8 都依赖提交 6——离线 demo 永不失败，
"重试"和"续跑省钱"只有接上真实模型才有意义。

### 提交 5：SRT 字幕源（外部层）✅ 已完成

**目标**：引擎能吃上真实的字幕文件，`vidrecap demo --srt 文件.srt` 用真字幕 +
离线模型跑通全流程（依然零 Key、零网络）。

| 文件 | 动作 |
|---|---|
| `vidrecap/external/adapters/srt/source.py` | 新建：解析 SRT（序号 + `00:00:01,000 --> 00:00:04,000` 时间戳 + 文本行），实现 `MediaSource` 插座；`duration()` 取最后一条字幕结束时间；`content(start, end)` 按绝对时间返回窗内字幕 |
| `vidrecap/external/adapters/srt/__init__.py` | 新建：挂出 `SrtSource` |
| `vidrecap/external/api/__init__.py` | 挂出 `SrtSource` |
| `vidrecap/user/cli.py` | `demo` 子命令加 `--srt 路径`：有值时媒体源换成 `SrtSource`，其余逻辑不动 |
| `tests/test_srt_source.py` | 新建：固定 sample 字幕，断言时间轴对齐、重叠窗取到重复内容、时长、坏文件报错、确定性 |
| `AGENTS.md` | 文件清单同步（CI 检查） |

SRT 解析纯标准库手写（按空行切块、正则认时间戳），不引依赖；容错口径：BOM、
`\r\n`、缺序号行要能吃下，时间戳解析失败直接报错（宁可不跑，不能静默错位）。
实现时踩的坑：U+FEFF（BOM）不算空白符，`str.strip()` 吃不掉，第一块的序号行
会认不出来——解析入口显式 `lstrip` 掉。

**验收**：已完成。新增 11 个测试、全库 209 个全绿；不带 `--srt` 时 demo 输出
与改前逐字节一致；`--srt` 同一文件跑两次逐字节一致；真实字幕端到端跑通，
跨窗字幕在相邻两窗都出现（重叠缓冲语义正确）。

### 提交 6：OpenAI 兼容大模型适配器（外部层）✅ 已完成

**目标**：接上第一个真实模型。任何 OpenAI 兼容端点（官方、DeepSeek、Qwen、Kimi…）
都能当 `LLMClient` 用，引擎其余部分一行不改——这就是插座设计的兑现时刻。

| 文件 | 动作 |
|---|---|
| `vidrecap/external/adapters/openai/llm.py` | 新建：`async summarize(text, instruction)` → `POST {base_url}/chat/completions`；base_url / api_key / model / timeout 从环境变量或参数读，不写死任何厂商 |
| `vidrecap/external/adapters/openai/__init__.py` | 新建：挂出 `OpenAICompatibleLLM` |
| `vidrecap/external/api/__init__.py` | 挂出 |
| `vidrecap/user/cli.py` | `demo` 加 `--llm openai`、`--model`、`--instruction`（自定义提示词，对应 JD 的"用户自定义 Prompt"；调优提示词属闭源资产，适配器只透传不内置） |
| `vidrecap/data/models.py` | `PipelineConfig` 加 `summarize_instruction`（默认值 = 原先硬编码在编排器里的指令） |
| `vidrecap/service/orchestrator.py` | 分片摘要改用 `cfg.summarize_instruction` |
| `tests/test_openai_llm.py` | 新建：本地起标准库假 HTTP 服务，测请求组装、响应解析、超时、非 200 报错——**不打真网络** |
| `AGENTS.md` | 文件清单同步 |

**依赖决策（默认零依赖）**：HTTP 用标准库 `urllib.request` + `asyncio.to_thread` 包成异步，
不加任何新包。若后续需要流式输出/连接池，再提 `httpx` 走"加依赖前先问"流程。

**可选项（公信力卖点）**：同一套打分考卷换 LLM 打分器重跑，README 公布
"启发式 vs 大模型"成绩对比。考卷与指标在提交 2 就为这一天设计的，零改造。

**实现时的三处调整**

1. `--instruction` 要真正生效，得从命令行一路布线到模型调用——按铁律 3，
   分片摘要指令的默认值从编排器的硬编码搬进 `PipelineConfig.summarize_instruction`，
   命令行只做覆盖（默认路径输出逐字节不变）。
2. **安全口径（安全工具拦过一次，结论保留）**：适配器是"使用者自己配置端点"的
   客户端，不接收不可信输入构造 URL，因此**不做私网地址封禁**——连本地 ollama、
   内网 vLLM 网关恰恰是核心用途；作为兜底，只允许 http/https 协议且不跟随重定向。
3. 适配器暴露只读 `model` 属性，命令行据此显示实际生效的模型名（含来自
   环境变量的情况）。

**验收**：已完成。新增 10 个测试、全库 221 个全绿；默认 demo 输出逐字节一致；
本地假服务上端到端跑通 `--llm openai`（含无模型名时的明确报错）。

### 提交 7：失败重试与降级（服务层）✅ 已完成

**目标**：真实模型会超时、会限流，单个分片失败不能炸掉整个任务，也不能静默丢内容。

| 文件 | 动作 |
|---|---|
| `vidrecap/service/orchestrator.py` | LLM 调用包上指数退避重试（次数、初始等待可配，默认 3 次 × 0.5s 起）；重试耗尽按 `PipelineConfig` 新开关走：跳过该分片并记入 `RecapStats.failed_shards`（默认），或整体抛错 |
| `vidrecap/data/models.py` | `PipelineConfig` 加 `max_retries` / `retry_initial_delay` / `retry_backoff` / `on_shard_failure`（默认值只在这里声明一次）；`RecapStats` 加 `failed_shards` |
| `tests/test_orchestrator.py` | 扩充：假 LLM 前 N 次抛错再成功 → 重试后成功且调用次数正确；始终抛错 → 跳过 + 统计正确；策略为抛错模式 → 整体失败 |

重试手写约二十行，不引 tenacity（docs/REUSE.md 里有替换方案备着）。归属表里
"失败重试"明确写在服务层——它是执行关切，不进规划层。

**实现时的边界决定**：重试只包**分片摘要**这一处调用。压缩合并阶段（`compress`
里的 `summarize`）不加跳过——合并步骤没有"跳过一片"的语义，失败就整任务失败；
要给它加重试，等真实使用中那里真的开始失败再说（YAGNI）。

**验收**：已完成。新增 4 个测试、全库 225 个全绿；瞬时失败 1 次重试后成功
（调用次数断言）；持续失败默认跳过并计入 `failed_shards`、概括为空不崩；
`raise` 策略整任务失败；`max_retries=0` 时每片恰好调用一次；默认 demo 输出
逐字节一致。

### 提交 8：断点续跑（数据层 + 服务层）✅ 已完成

**目标**：真实模型按量计费，3 小时视频几十个分片，中途挂掉不能从头烧一遍钱。

| 文件 | 动作 |
|---|---|
| `vidrecap/data/store.py` | 新建：sqlite3（标准库）存分片结果；任务指纹 = 输入内容指纹，内容没变才能复用 |
| `vidrecap/data/api/__init__.py` | 挂出 `TaskStore` |
| `vidrecap/service/orchestrator.py` | 每片摘要落库；`--resume`（或参数）时先查库，已完成的分片直接复用，不重新调模型 |
| `vidrecap/user/cli.py` | `demo` 加 `--store 路径`（给了才启用持久化，默认关闭，保持零依赖开箱即用） |
| `tests/test_store.py` + `tests/test_orchestrator.py` 扩充 | 新建：假 LLM 第 3 片抛错 → 重跑 → 前 2 片调用计数为 0（没重新调模型）→ 最终输出与一次跑完一致；内容指纹变了必须全部重跑 |
| `AGENTS.md` | 文件清单同步 |

**实现时的两个设计决定**

1. **缓存是内容寻址的**：键 = 命名空间（模型/提示词）+ 分片原文哈希。原文改一个字
   或换个模型就整片重算；反过来，不同时长的源只要前几片逐字相同就直接命中
   （测试里 1.5h 的源复用了 1.0h 跑出的 5 片——这正是内容寻址的语义，不是巧合）。
2. 缓存存的是**质检之后的最终结果**（含平均分与修正数），命中后连打分器都不用跑。

**验收**：已完成。新增 4 个测试、全库 230 个全绿；中断（4 片失败）重跑只调 4 次
模型且结果与一次跑完一致；同输入重跑零调用；换命名空间全部重算；质检字段经
缓存往返无损；不传 `--store` 时行为与改前完全一致（默认 demo 输出逐字节一致）。

### 阶段二收尾

提交 5–8 全部完成。命令行现在是：`vidrecap demo --srt 节目.srt --llm openai
--model 模型名 --store cache.db`——真实字幕进、真实模型算、挂了接着跑。

### 阶段二统一验收

1. 8 个提交做完后，`.venv/Scripts/python.exe -m pytest -q` 全绿（分层硬检查含新文件清单）；
2. `vidrecap demo` 默认输出全程与基准逐字节一致（零 Key 底线的证据）;
3. 有真实字幕 + 有 Key 时：一条命令端到端出长视频概括；
4. 每个提交同步 `AGENTS.md` 文件清单与归属表。

---

## 阶段三（提交 9–12）：JD 对齐——人物权重、技能配置、多模态

目标：补齐 JD 明确要求的三件事（Function Calling 取字幕与人物标签并按权重取舍、
skill-md 配置 + 双重提示词约束、画面与人物动作），依旧守住零 Key 可跑与确定性两条底线。

### 提交 9：后台取数插座 + 人物数据模型（外部层 + 数据层）✅ 已完成

- `external/protocols.py` 新插座 `ContentCatalog`：给时间段，还"带说话人的字幕行 + 人物档案"
- `data/models.py`：`SubtitleLine`（start/end/text/speaker）、`SpeakerProfile`（name/tier/appearances）
- 外部层适配器：对接客户后台（HTTP 或数据库，**SQL 一律参数绑定**）；demo 假后台用 sqlite
  预置大咖与配角，零 Key 可演示
- **Function Calling 口径**：核心链路走适配器确定性取数（编排器知道何时取哪段，不需要
  模型自己决定）；"模型自主发起工具调用"只做可选演示形态——理由：省 token、稳定、可复现

**验收**：已完成。新增 8 个测试、全库 243 个全绿；demo 假后台（内存 sqlite，
参数化查询）预置大咖约三成发言并统计出场次数；取窗语义与 SrtSource 一致
（有交集即命中）；确定性双实例逐项相等；默认 demo 输出逐字节一致。
对接真实客户后台的适配器按开源边界留白（闭源侧）。

### 提交 10：人物权重策略（规划层 + 服务层）✅ 已完成

- `planning/speakers.py`：`plan_speaker_policy` 纯函数——大咖/高频优先保留，低频配角过滤；
  **考卷先行**，策略考卷进 monitor/evals
- 服务层分片组装改从目录取数、按计划执行
- 阈值默认值只进数据层配置（铁律 3）

**实现时的两个设计决定**

1. **"频率过低"用相对占比（台词份额）而非绝对次数**：绝对次数对长视频不收敛
   （说了一小时的常驻嘉宾和说了十分钟的嘉宾都是"几百次"），占比才符合
   "配角"的直觉。默认阈值 0.05，大咖与无标签发言豁免，无档案源整体跳过策略。
2. **偏离说明**：策略考卷没用 monitor/evals 的 JSONL，直接写在
   `tests/test_speakers.py`——输入是结构化对象，套 JSONL 是仪式感；
   与提交 4 砍掉 scenarios.py 同一条 YAGNI 理由。

**验收**：已完成。新增 7 个测试、全库 252 个全绿；边界（占比恰等于阈值）保留；
`vidrecap demo --catalog` 端到端跑通：群众演员（1 句）被过滤且不进任何摘要，
大咖发言保留，统计行可见；默认 demo 输出逐字节一致。

### 提交 11：skill-md 技能配置 + 双重提示词（用户层 + 数据层 + 外部层）✅ 已完成

- `data/models.py`：`SkillConfig`（system_prompt、summarize_instruction、质量参数、人物策略引用）
- 用户层（装配根）解析 skill-md：受限 frontmatter 格式，**不引 YAML 依赖**
- `LLMClient.summarize` 加可选 `system` 参数（默认空、向后兼容），两级约束分别成消息
- CLI 加 `--skill 文件.md`；优先级写死：命令行 > 技能文件 > 默认值

**实现时的两个关键决定**

1. **指令语义修正**：原先 `instruction` 被当成 system 消息发出（历史遗留）；现在
   `system` = 角色与规则，`instruction` = 本次要求，各成一条消息，正文最后。
   实测发现这会打挂"按老签名写的适配器"，于是加了**向后兼容**：system 为空时
   **根本不传该参数**——测试里那些只认 instruction 的假模型不用改一行就照跑，
   这正是客户自写适配器会遇到的场景。
2. 技能文件格式对齐 **Agent Skills 开放标准**（头部元数据 + 正文指令），
   解析只认受限平铺格式（缩进行续上一键），不引 PyYAML；示例见
   `skills/新闻摘要.md`。

**刻意不做**：压缩合并阶段不带 system 提示词（只包分片摘要）——要让全局概括也吃
系统提示词，得把配置传进 `compress`，等真实使用提出需求再加。

**验收**：已完成。新增 15 个测试、全库 267 个全绿；优先级三档（命令行 > 技能 > 默认）
逐项断言；双重提示词实测到达模型（记录式假模型 + 假 HTTP 服务三条消息）；
`vidrecap demo --skill skills/新闻摘要.md` 跑通；默认 demo 输出逐字节一致。

### 提交 12：画面与人物动作（外挂为主，成本敏感）✅ 已完成

- 外挂脚本扩展：抽帧（默认稀疏、场景切换加密、密度可配）→ 视觉模型产"谁在做什么"事件行
- 事件行与语音字幕按时间对齐，经 `MediaSource` 适配器进同一条流水线
- 合并阶段提示词升级：带人物线/剧情线约束的全局概括
- 按 帧 计费是主要成本，放在前三件跑顺真实视频之后再做

**实现方式与三处偏离**

1. **抽帧用 ffmpeg 固定间隔**（默认 120 秒、上限 200 帧，运行前打印帧数供估算），
   **场景切换加密未做**：没装 ffmpeg 的机器上场景检测没法验证，而固定间隔的
   成本一眼可估。PySceneDetect（BSD-3）已在 REUSE.md 备着，需要时再接。
2. **画面事件合进字幕轨**，不新建插座：视觉描述写成带 `〖画面〗` 前缀的字幕行，
   时间对齐后与语音行一起排序——分片、质检、人物策略、断点续跑全部照旧生效，
   零改动复用整条流水线。
3. **视觉能力挂在 OpenAI 适配器的 `describe_image` 方法上**（图片内联成 data URI），
   不为它单建插座——只有一个实现，按 YAGNI 不引入抽象。
4. **合并阶段的"剧情线"提示词升级未做**：与提交 11 的保留项同因（要把配置传进
   `compress`），等真实视频提出需求再加。

**验收**：已完成。全库 270 通过、1 跳过（抽帧测试需 ffmpeg，本机没有、
CI 的 Ubuntu 自带）；轨道合并、参数转发、看图消息组装均有断言；
默认 demo 输出逐字节一致。

---

## 提交 13：服务化外壳（用户层新子目录）✅ 已完成

把引擎从"一次性命令行"变成"常驻服务"：调用方 POST 一个摘要请求，进度边跑边推，
跑完返回结果。

| 文件 | 动作 |
|---|---|
| `vidrecap/user/server/server.py` | 新建：标准库 http.server + SSE；`POST /recap` 流式返回 `progress`/`result`/`error`，`GET /health` 探活 |
| `vidrecap/user/server/__init__.py` | 新建：挂出 `serve` / `build_server` / `RecapRequest` |
| `vidrecap/user/assembly.py` | 新建：适配器选型从 cli 提炼出来，命令与服务端共用一份（两处各写一份迟早会漂） |
| `vidrecap/user/api/__init__.py` | 挂出服务入口 |
| `vidrecap/user/cli.py` | 新增 `serve` 子命令；改用 assembly 的装配函数 |
| `tests/test_server.py` | 新建：真起服务、逐行读事件流、404/400/429 路径 |

**为什么先不上 gRPC**：服务化的价值在"常驻、被调用、推进度"这个形态，不在协议。
零依赖的 HTTP + SSE 就能把形态做完整；等有了明确的 gRPC 生态对接方，
再按"加依赖先问"换实现——接口契约到那时已经稳定，换实现是局部改动。

**三条有意为之的边界**

1. **默认只绑 127.0.0.1**：外壳不含鉴权，对外暴露请自行加反向代理与鉴权；
2. **同时最多 2 个任务**（超了回 429，不排队）：防止一次涌进来的请求把机器打满；
3. **请求模型逐字段校验**（负时长、拼错字段名一律回 400）：服务是信任边界，
   坏参数不许进流水线。

**验收**：已完成。全库 278 通过、1 跳过；真机探针：命令行起服务 → 外部客户端调用 →
拿到 2 次进度事件 + 结果（`--catalog` 下配角被过滤的效果在结果里可见）；
默认 demo 输出逐字节一致。

---

## 提交 14：操作台三件套——页面、历史记录、视频直传 ✅ 已完成

把提交 13 的"裸 HTTP 外壳"补成上手即用的工具：浏览器里一个单文件操作台，
跑成功的任务自动归档可回看，视频/音频文件直接选中分析（服务端语音转写字幕）。

| 文件 | 动作 |
|---|---|
| `vidrecap/user/server/page.html` | 新建：单文件操作台（零依赖、无构建；双栏布局，右侧历史侧栏）；EventSource 不支持 POST，SSE 用 fetch 流手工分帧 |
| `serve.bat` / `serve.sh` | 新建：一键启动（必须用项目虚拟环境解释器——系统 python 没装 pydantic） |
| `vidrecap/data/history.py` | 新建：`RunRecord` + `HistoryStore`——JSONL 追加式任务档案；与 TaskStore 分工：那个是**缓存**（内容寻址、可作废），这个是**档案**（只追加、不参与计算） |
| `vidrecap/external/adapters/whisper/` | 新建：faster-whisper 适配器（可选依赖、模型延迟加载、`to_srt_text` 纯函数）；`external/protocols.py` 增 `Transcriber` 插座 |
| `vidrecap/external/adapters/srt/source.py` | `SrtSource` 支持内联文本（`text=`）：操作台的字幕随请求体带来，不落盘 |
| `vidrecap/user/server/server.py` | `RecapRequest` 增连接信息/内联字幕/视频字段；`GET /history(/{id})` 回看；`POST /upload` 收视频（原始字节 + 文件名消毒，2GB 上限）；带 `video` 先转写再进流水线，进度事件带 `stage`（转写=秒数、摘要=分片数） |
| `vidrecap/user/assembly.py` | `build_transcriber`；`build_llm` 收页面当场填的连接信息 |
| `vidrecap/user/cli.py` | serve 新旗标：`--history/--no-history`、`--whisper-model/--no-whisper` |
| tests | `test_history.py`、`test_whisper.py` 新建；`test_server.py` 补历史/上传/视频/错误路径 |

**边界三条**

1. **faster-whisper 仍是可选依赖**：没装时视频入口明确报错指路，`--no-whisper` 可关——
   "运行时只依赖 pydantic"的底线不破；
2. **历史只记成功任务**（失败那次在事件流里已说清）；归档落盘失败不影响已推送的结果；
3. **画面/人物动作（`--visual`）仍走外挂脚本**：需要 ffmpeg 二进制与视觉模型，
   服务端暂不接（场景切换加密抽帧见"更远的路线"）。

**验收**：已完成。全库 300 通过、1 跳过；真机端到端：页面选 TTS 合成的 wav →
上传 → whisper 转写（进度按秒推）→ 摘要 → 历史区自动新增一条；`sintel.mp4`
（77MB、15 分钟开源短片）经命令行与 `/upload` 接口同链路验证。

---

## 提交 15：服务端画面轨——抽帧 + 视觉描述并入时间线 ✅ 已完成

把提交 12 外挂脚本里的 `--visual` 搬进服务端：视频直传时勾"画面分析"，
ffmpeg 抽帧 → 视觉模型逐帧描述 → 画面行带 `〖画面〗` 前缀并入字幕轨，
后面的概括流水线零改动复用（与外挂同一套口径：固定间隔抽帧、短时段占位）。

| 文件 | 动作 |
|---|---|
| `vidrecap/external/adapters/visual/frames.py` | 新建：`ffmpeg_exe`（系统 ffmpeg → imageio-ffmpeg 兜底 → 报错指路）、`extract_frames`（fps 滤镜均匀抽帧、640px 缩放）、`merge_tracks`（画面行并轨，纯函数） |
| `vidrecap/external/adapters/visual/describe.py` | 新建：`DemoVisionDescriber`——确定性离线占位（明确标注"离线演示"，不冒充真实画面理解），零 Key 链路完整 |
| `vidrecap/external/protocols.py` | 增 `VisionDescriber` 插座（`describe_image`）；真实实现复用 `OpenAICompatibleLLM.describe_image`（外部层已有） |
| `vidrecap/user/server/server.py` | `RecapRequest` 增 `visual/frame_interval/max_frames/vision_model`；转写后抽帧并轨，进度事件 stage="画面"（按帧推）；`visual` 无视频回 400 |
| `vidrecap/user/assembly.py` | `build_vision`：demo 引擎配演示描述器、openai 引擎复用兼容客户端（连接信息与摘要模型同源） |
| `vidrecap/user/server/page.html` | 高级选项：画面分析开关 + 视觉模型名；历史行带"画面"标记 |
| `vidrecap/data/history.py` | `RunRecord.visual` 入档案 |
| tests | `test_visual.py`（归并/确定性/协议形状/ffmpeg 解析/真实抽帧——无 ffmpeg 环境跳过）；`test_server.py` 补画面轨端到端（抽帧打桩）与 400 路径 |

**边界**

- ffmpeg 同 faster-whisper 一样是**可选装配**（系统版或 imageio-ffmpeg 任一），
  测试里真实抽帧一条按可用性跳过，CI 不被迫装它；
- demo 描述器只证明链路，真实画面理解必须配 Key（页面上填视觉模型名即可，
  如 glm-4v-flash / gpt-4o-mini）；
- 场景切换加密抽帧（PySceneDetect）仍在远期路线，当前是固定间隔。

**验收**：全库 310 通过、1 跳过（1 条真实抽帧在本机跑、CI 无 ffmpeg 时跳过）；
真机端到端：sintel.mp4 + 画面分析（demo 描述器）→ 时间线含 `〖画面〗` 行。

---

## 提交 16：说话人分离——声纹聚类给字幕贴"谁在说"标签 ✅ 已完成

补上"人物"这格的最后一块拼图：视频直传时勾"说话人分离"，sherpa-onnx 做
声纹聚类（音频切窗 → 每窗算嗓音指纹 → 相近的聚成一个人），字幕行按
"与哪段的说话时间重叠最长"贴上"说话人N："标签——概括结果从"说了什么"
变成"谁说了什么"。**只分人不认名**：聚类对上真名（声纹登记/按内容推断）留在远期。

| 文件 | 动作 |
|---|---|
| `vidrecap/external/adapters/diarize/separate.py` | 新建：`SherpaDiarizer`（sherpa-onnx 可选依赖、模型延迟加载、ffmpeg 解码 16k 单声道）+ `apply_speakers` 贴标签纯函数；模型首次使用自动下载到 `.vidrecap/models/`（免令牌直链，**下载只走 https + 开源托管域名白名单 + 重定向逐跳校验**——安全闸门要求的 SSRF 防护） |
| `vidrecap/external/protocols.py` | 增 `Diarizer` 插座 |
| `vidrecap/user/server/server.py` | `RecapRequest.diarize`；视频三步走：转写 → 分人贴标签 → 画面轨（画面行不贴"说话人"）；进度事件 stage="分人"；无视频回 400 |
| `vidrecap/user/assembly.py` | `build_diarizer`（threshold 聚类松紧，默认 0.5） |
| `vidrecap/user/server/page.html` | 说话人分离开关；历史行带"分人"标记 |
| `vidrecap/data/history.py` | `RunRecord.diarize` 入档案 |
| tests | `test_diarize.py`（贴标签/协议形状/下载白名单/真实静音分离——栈不齐跳过）；`test_server.py` 补分人端到端与错误路径 |

**边界**

- sherpa-onnx 与 faster-whisper、ffmpeg 同为**可选依赖**（pip 装、模型 ~46MB 首用下载）；
- 分离是纯本地声学计算，**零 Key**；"语气"不归它管（声纹不带情绪）；
- 与人物权重（`--catalog`）的联动暂未做：分离出的匿名说话人走"匿名保留"规则，
  按占比过滤匿名聚类是后续设计题。

**已知局限（实测）**：干净语音分离准确（单说话人 TTS wav → 1 人 7 段，句间停顿处断开）；
**音乐/音效占比高的稀疏语音会过分裂**——Sintel 全片（约 85% 时长是配乐）默认阈值分出
约 20 个"说话人"，调大聚类阈值（0.5→0.7）只收敛到 19，治标不治本。正解是"按转写出的
语音段裁音频再聚类"（不喂音乐段进声纹），记入远期路线。

**验收**：全库 319 通过、1 跳过；真机四阶段（转写→分人→画面→摘要）进度全通，
sintel.mp4 字幕带"说话人N："标签（含上述过分裂局限）。

---

## 提交 17：说话人分离的语音段裁剪——词级时间戳 + 只对语音聚类 ✅ 已完成

照 whisperX 的套路修提交 16 实测的过分裂：**音乐/音效段从源头不进声纹聚类**。

两刀：

1. **转写词级时间戳**（`whisper/transcribe.py` 开 `word_timestamps=True`）：
   句级时间戳在长静音处会被拉得极长（sintel 实测一句标了 217 秒，大部分是
   配乐），按首尾词的实测时间把句子起止收紧——顺带把 SRT 时间轴和分片取材
   都修准了（语音段从 344 秒缩到 55 秒）；
2. **分离按语音段裁剪**（`diarize/separate.py`）：调用方把转写句段传进
   `speech_spans`，加余量、合并重叠、裁剪拼接后只对语音聚类，算出的时间用
   线性映射映回原时间轴——接口不变，调用方无感。

**实测（sintel 全片，最难例：约 85% 时长是配乐、气声对白）**：说话人数
**约 24 → 5-9**（默认阈值 5 人，阈值 0.75 时 6 人；理想 2-3）；干净语音依旧
准确。剩余差距是声纹嵌入在电影级难例上的天花板，换更强嵌入模型
（wespeaker / pyannote 全家桶）见远期。

**验收**：全库 321 通过、1 跳过；服务端四阶段（转写→分人→画面→摘要）复测通过。

---

## 提交 18：方言识别引擎——SenseVoice 双引擎装配 ✅ 已完成

补方言短板（whisper 对粤语/四川话等识别率断崖）：接入 SenseVoice
（FunASR 生态、Apache-2.0，中/粤/英/日/韩）作第二识别引擎。**零新增依赖**——
经 sherpa-onnx 的 ONNX 导出运行（说话人分离已在用的库），模型三件
（int8 约 230MB + tokens + Silero VAD）首次使用自动下载，走提交 16 的白名单通道。

| 文件 | 动作 |
|---|---|
| `vidrecap/external/adapters/sensevoice/transcribe.py` | 新建：`SenseVoiceTranscriber` 实现 `Transcriber` 插座。SenseVoice 不出时间戳 → 先 Silero VAD 窗口循环切人声段、逐段识别、段起点即时间戳（音乐/静音挡在识别外）；语言令牌映射 auto/zh/yue/en |
| `vidrecap/user/assembly.py` | `build_transcriber` → `build_transcribers`：两套引擎一起装配（都延迟加载，启动零成本），请求按名字挑 |
| `vidrecap/user/server/server.py` | `RecapRequest.asr`（请求级引擎选择，默认 whisper；装配缺席时报错指路） |
| `vidrecap/user/server/page.html` | 识别引擎下拉（Whisper / SenseVoice） |
| `vidrecap/user/cli.py` | `--no-whisper` 文案更新；引擎选择从启动旗标改为请求字段（页面即点即用） |
| tests | `test_sensevoice.py`（协议形状 + VAD 切段逻辑——假 VAD 验证余量合并/最小语音时长丢弃） |

**设计取舍**：引擎选择从"启动旗标"改成"请求字段"——转写器本来就延迟加载，
双引擎同装零成本，页面切换不用重启服务；whisper 仍是默认（普通话/英文够用），
方言素材选 SenseVoice。Qwen3-ASR（22 种方言、云端）记入远期。

**验收**：全库 325 通过、1 跳过；真机：SenseVoice 官方中文样例 VAD 切段
准确、识别文本正确。**粤语对比实测**（官方 yue.mp3 样例，5 秒）：
SenseVoice 转"几个字都表达**唔**到我想讲**嘅**意思"（保留方言虚词 唔/嘅），
whisper tiny 转成书面化"表达**不到**…**的**意思"（方言特征丢失）——
后者对后续"人物语气/方言风味"的分析是信息损失；两引擎耗时相当（4s）。

---

## 提交 19：任务受理制——断连存活、失败入档、转写缓存 ✅ 已完成

修"浏览器标签页开着 = 任务活着"这条命脉：原来任务在请求线程里同步跑、
进度逐条往连接写——刷新页面/断网，下一次写进度就报错把任务一起带死，
死在半路的任务连历史档案都进不去。1 小时实测两次撞上（进程死亡 + 结果丢失无痕）。

三刀：

1. **受理制**：`POST /recap` 校验后立即回 202 + job_id，任务搬进后台线程
   照跑到底；进度回调从"写连接"改成"写登记簿"（内存 `_Task` 表，带锁）；
   新增 `GET /jobs/<id>` 领进度与结果。页面每秒轮询、job_id 存 localStorage，
   **刷新后自动接回**（服务重启丢任务时明确提示）。流水线与装配零改动——
   改的只是"进度往哪送、结果给谁"。
2. **失败入档**：`RunRecord` 加 `status`/`error`、`stats` 可空；归档挪进任务
   线程的收尾——成功记成果，失败记遗书，历史区带"失败"徽标、详情可看原因。
3. **两级缓存**：转写字幕按"路径+大小+mtime"哈希落盘（`.vidrecap/uploads/<键>.srt`），
   同一视频重跑秒过转写（1 小时素材省 7 分钟）；分片断点续跑（TaskStore，
   命令行早就在用）默认挂上——真模型重跑时已完成分片不再花钱。

附带（同属可靠性）：说话人分离加 30 分钟时长守卫——超限明确报错指路，
不再被内存压力无声杀死（1 小时实测两次进程死亡后的结论）；裁剪后及时
释放解码样本。

| 文件 | 动作 |
|---|---|
| `vidrecap/user/server/server.py` | `_Task` 登记簿 + 受理制 + `GET /jobs/<id>` + `_run_task_thread`（后台线程、归档收尾、信号量搬家）；转写缓存；默认分片缓存 |
| `vidrecap/data/history.py` | `RunRecord.status/error`、`stats` 可空 |
| `vidrecap/user/server/page.html` | 轮询替代 SSE、刷新续接、失败徽标与失败详情 |
| `vidrecap/external/adapters/diarize/separate.py` | 30 分钟时长守卫 + 裁剪后释放样本 |
| `vidrecap/external/api/__init__.py` | 导出 `parse_srt`（缓存命中的字幕重新解析出语音段） |
| tests | `_stream_events` 改为"提交即断连、轮询收尾"（每条用例都顺带验证断连存活）；新增断连存活/任务 404/转写缓存命中/失败入档四类用例 |

**验收**：全库 329 通过、1 跳过；真机：20 分钟转写任务受理后立即断开客户端、
刷新页面自动接回实时进度、2.5 分钟后结果在页面渲染、历史区自动入档。

**重启自愈（同日补遗，用户拍板"修复一下"）**：登记簿原本只在内存，服务重启
丢掉全部任务号。新增数据层 `data/jobs.py`（`JobStore`：一任务一 JSON 文件、
原子写、损坏当 404）——任务受理后写穿落盘；服务重启后已完成的任务仍可凭号
取结果，遗留的 running 任务由 `mark_interrupted` 标成 interrupted 并指路重提交
（页面加 interrupted 分支）；任务号校验形状（12 位十六进制），不给路径穿越留门。
`serve` 默认装配任务簿（`.vidrecap/jobs/`）。真机验收：提交 → 重启服务 →
凭号取到结果；测试 333 通过、1 跳过。

---

## 更远的路线（阶段四及以后，先不展开）

| 功能 | 落哪层 | 说明 |
|---|---|---|
| 更强声纹嵌入（wespeaker / pyannote 全家桶） | 外部层 | 电影级难例（配乐垫底、气声对白）说话人收敛的最后一块；提交 17 已把架构与数据打通 |
| 监控指标导出与告警 | 监控层 | 结构化日志先行 |
| gRPC 版服务（替换 HTTP 外壳） | 用户层 | 有明确对接方时再引 grpcio，接口契约已稳定 |
| 场景切换加密抽帧 | 外挂脚本 | 接 PySceneDetect（BSD-3，见 REUSE.md）；固定间隔的服务端画面轨已在提交 15 落地，此处是场景感知的密度升级 |
| ASR 音频直转（包内适配器） | 外部层 | **提交 14 已落地**：faster-whisper 可选装配、服务端视频直转；外挂脚本 `scripts/video2recap.py` 仍保留（带 `--visual` 画面轨） |

每加一项，先确认它属于哪层、是否需要新插座，再补 `AGENTS.md` 的归属表与文件清单。

---

## 常用验收命令

```bash
# 全部测试（含分层硬检查）
.venv/Scripts/python.exe -m pytest tests/ -q

# 离线 demo（需要证明"行为不变"时与改前输出逐字节对比）
.venv/Scripts/python.exe -m vidrecap demo > /tmp/after.txt
diff /tmp/baseline.txt /tmp/after.txt

# 跑评测集（提交 2 之后可用）
.venv/Scripts/python.exe -m vidrecap eval
```
